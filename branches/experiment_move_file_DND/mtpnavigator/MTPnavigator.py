...
DND_TARGET_EXTERN_FILE=0
DND_TARGET_INTERN_FILE=1
...
class MTPnavigator:
    ...
    def __setup_drag_and_drop(self):
        ...
        target_extern = ('text/uri-list', 0, self.DND_TARGET_EXTERN_FILE)
        self.__treeview_track.drag_dest_set(gtk.DEST_DEFAULT_ALL, [target_extern], gtk.gdk.ACTION_COPY | gtk.gdk.ACTION_MOVE)
        self.__treeview_file.drag_dest_set(gtk.DEST_DEFAULT_ALL, [target_extern], gtk.gdk.ACTION_COPY | gtk.gdk.ACTION_MOVE)
        
        # intern drag and drop into file view (move files and folders)
        target_intern = ('MTPNAV_FILE', gtk.TARGET_SAME_WIDGET, self.DND_TARGET_INTERN_FILE)
        self.__treeview_file.drag_source_set(BUTTON1_MASK, [target_intern], gtk.gdk.ACTION_MOVE)
        self.__treeview_file.drag_dest_set(gtk.DEST_DEFAULT_ALL, [target_extern, target_intern], gtk.gdk.ACTION_MOVE)

        # connect events        
        self.__treeview_track.connect('drag_data_received', self.on_drag_data_received)
        self.__treeview_file.connect('drag_data_get', self.on_drag_data_get)
    ...
    def on_drag_data_get(self, treeview, context, selection_data, info, time, data):
        print "****** on_drag_data_get_cb"
        print context
        ...
        print data
        print "**************************"
        #FIXME: slection can be multiple
        model, iter = treeview.get_selection().get_selected()
        metadata = model.get_metadata_from_iter(iter)
        selection_data.set(selection.target, 8, metadata)
